...
const MenuManager = (() => {
    ...
    function defaultSettings() {
        return {
            ...
            joystickScale: 1.0,
            minimapScale: 1.0,
            minimapFps: (typeof getDefaultMinimapFps === 'function') ? getDefaultMinimapFps() : 60,
            graphicsQuality: (typeof getDefaultGraphicsQuality === 'function') ? getDefaultGraphicsQuality() : 'highest'
        };
    }
    ...
    function loadSettings() {
        ...
        try {
            ...
            if (typeof GRAPHICS_PRESETS !== 'undefined' && !GRAPHICS_PRESETS[merged.graphicsQuality]) {
                merged.graphicsQuality = defaults.graphicsQuality;
            }
            merged.minimapFps = (typeof normalizeMinimapFps === 'function')
                ? normalizeMinimapFps(merged.minimapFps)
                : ([15, 30, 40, 60].includes(parseInt(merged.minimapFps, 10)) ? parseInt(merged.minimapFps, 10) : defaults.minimapFps);
    ...
    function applySettings() {
        ...
        const graphicsHint = document.getElementById('setting-graphics-hint');
        const minimapFpsSelect = document.getElementById('setting-minimap-fps');

        if (graphicsSelect) graphicsSelect.value = s.graphicsQuality || ((typeof getDefaultGraphicsQuality === 'function') ? getDefaultGraphicsQuality() : 'highest');
        if (minimapFpsSelect) minimapFpsSelect.value = String(s.minimapFps ?? ((typeof getDefaultMinimapFps === 'function') ? getDefaultMinimapFps() : 60));
        if (graphicsHint && typeof GRAPHICS_PRESETS !== 'undefined') {
            const preset = GRAPHICS_PRESETS[graphicsSelect ? graphicsSelect.value : s.graphicsQuality];
            if (preset) graphicsHint.textContent = preset.description;
        }
        ...
        // Minimap FPS terpisah dari Graphics Quality dan berlaku di PC/Mobile.
        if (typeof setMinimapFps === 'function') setMinimapFps(s.minimapFps);
    }
    ...
    function initSettingsEvents() {
        ...
        if (graphicsSelect) {
            graphicsSelect.addEventListener('change', () => {
                ...
                if (typeof GRAPHICS_PRESETS !== 'undefined' && !GRAPHICS_PRESETS[quality]) return;
                saveSettings({ graphicsQuality: quality });
                const preset = (typeof GRAPHICS_PRESETS !== 'undefined') ? GRAPHICS_PRESETS[quality] : null;
                if (graphicsHint && preset) graphicsHint.textContent = preset.description;

                // Terapkan preset LANGSUNG tanpa reload halaman: renderer dibuat
                // ulang, resolusi minimap & interval visual disesuaikan, lalu
                // level yang sedang aktif dibangun ulang dengan shader/material
                // baru (lihat applyGraphicsQualityLive di game.js).
                let applied = false;
                if (typeof applyGraphicsQualityLive === 'function') {
                    try {
                        applied = applyGraphicsQualityLive(quality);
                    } catch (e) {
                        applied = false;
                        console.error('Apply graphics quality live gagal:', e);
                    }
                }

                if (applied) {
                    showNotificationMenu(`Graphics: ${preset ? preset.label : quality}. Applied!`);
                } else {
                    // Fallback: reload (jika fungsi apply live tidak tersedia /
                    // gagal — renderer antialias, precision, dll. hanya bisa
                    // dipastikan lewat boot ulang).
                    showNotificationMenu(`Graphics: ${preset ? preset.label : quality}. Reloading...`);
        ...
        if (minimapFpsSelect) {
            minimapFpsSelect.addEventListener('change', () => {
                const fps = (typeof normalizeMinimapFps === 'function')
                    ? normalizeMinimapFps(minimapFpsSelect.value)
                    : parseInt(minimapFpsSelect.value, 10);
                minimapFpsSelect.value = String(fps);
                saveSettings({ minimapFps: fps });
                if (typeof setMinimapFps === 'function') setMinimapFps(fps);
                showNotificationMenu(`Minimap: ${fps} FPS`);
            });
        }
