...
// DPR tinggi (2x/3x) sangat mahal pada ponsel. 1.35 tetap cukup tajam tetapi
// jauh lebih ringan, sehingga mengurangi black-screen/context-lost di GPU mobile.
renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, IS_MOBILE_DEVICE ? 1.35 : 2));
...
// Minimap Setup (High resolution internal canvas for crisp rendering)
const minimapCanvas = document.getElementById('minimap');
const minimapCtx = minimapCanvas.getContext('2d');
minimapCanvas.width = 480;
minimapCanvas.height = 480;
...
function loadLevel(lvl) {
    ...
    const playerLight = new THREE.PointLight(0xfffaed, 1.25, 22, 1.2);
    ...
    // Build Walls and Collect Empty Cells
    let emptyCells = [];
...
function createVoidMonsterMesh(accentHex, index) {
    ...
    const core = new THREE.Mesh(new THREE.SphereGeometry(R * 0.5, 18, 18), coreMat);
    ...
    const shell = new THREE.Mesh(new THREE.SphereGeometry(R * 1.0, 40, 40), shellMat);
    shell.scale.set(1, 0.9, 1);
    group.add(shell);

    // --- Gyroscopic void rings ---
    const ring1 = new THREE.Mesh(new THREE.TorusGeometry(R * 1.18, R * 0.05, 8, 32), ringMat);
    ring1.rotation.x = Math.PI / 2.6 + index * 0.3;
    const ring2 = new THREE.Mesh(new THREE.TorusGeometry(R * 1.32, R * 0.04, 8, 32), ringMat);
    ring2.rotation.x = Math.PI / 4.2;
    const ring3 = new THREE.Mesh(new THREE.TorusGeometry(R * 0.95, R * 0.03, 8, 32), ringMat);
    ...
    for (let o = 0; o < orbCount; o++) {
        const orb = new THREE.Mesh(new THREE.SphereGeometry(R * (0.05 + (o % 3) * 0.012), 8, 8), particleMat);
        orb.castShadow = true;
        group.add(orb);
        orbitals.push({
            g: orb,
    ...
    // --- Aura light ---
    const light = new THREE.PointLight(accentHex, 1.15, 8);
    group.add(light);
    const innerLight = new THREE.PointLight(0xffffff, 0.35, 3.5);
...
function spawnCrystalPickupFX(position, type = 'normal') {
    ...
    const shardCount = type === 'normal' ? 14 : 22;
...
function updateMonsters(delta) {
    ...
    if (packHuntTimer <= 0) {
        const cfg = getDifficultyConfig();
        packHuntTimer = cfg.huntInterval + Math.random() * 0.08;
...
function animate() {
    ...
    if (gameActive && !isPaused && !isDying && (controls.isLocked || isMobileControl)) {
        ...
        // Animasi kristal: melayang, rotasi multi-aksis, perputaran cincin orbital, dan pulsating core
        const t = time * 0.003;
